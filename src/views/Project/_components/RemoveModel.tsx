...
import { removeModel } from "../../../db/mutateData";
// @ts-ignore
...
export const RemoveModel: FC<RemoveModelProps> = ({ closeModal, modelId }) => {
  ...
  const modelObjToRemove = currentProject.models.find(
    (i: any) => i.modelUid === modelId
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await removeModel(
        currentProjectId,
        modelId,
        modelObjToRemove
      );
      console.log("Project Removed Response: ", response);
      history.push(`/project/${currentProjectId}`);
    } catch {
